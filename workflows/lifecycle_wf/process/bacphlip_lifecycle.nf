process bacphlip_lifecycle {
    ...
    output:
        ...
        tuple val(name), path("${name}_bacphlip_results.tsv"), emit: bacphlip_annotation, optional: true
    script:
        """
        bacphlip -i ${fasta} --multi_fasta

        mv ${name}*BACPHLIP_DIR ${name}_bacphlip_dir
        mv ${name}*bacphlip ${name}_bacphlip_results.tsv
        mv ${name}*bacphlip.hmmsearch.tsv ${name}_bacphlip_hmmsearch.tsv
